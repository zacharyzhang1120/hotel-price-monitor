...
如果你是从 GitHub 新拉取本项目，请先阅读：

```text
docs/HANDOFF_TO_WORKBUDDY.md
```

继续开发时优先遵守：

- 采用小步 Loop，不要大规模重构。
- 每轮改动都要测试，并把结果记录到 `docs/OPTIMIZATION_LOG.md`。
- 不要提交 `.env`、数据库、携程登录态、私钥、构建产物和诊断截图。
- 当前真实抓取优先优化携程，不要同时扩展多个 OTA 平台。
