...
class QunarScraper(BaseScraper):
    ...
    async def fetch_calendar(self, hotel_url: str, check_in_dates: list[date]) -> list[PricePoint]:
        if not hotel_url:
            raise ValueError("Qunar hotel_url is required")
    ...
    async def _fetch_single_date(self, page, hotel_url: str, check_in_date: date) -> PricePoint:
        target_url = with_qunar_date_params(hotel_url, check_in_date)
        try:
            ...
            raise TimeoutError(f"Qunar page timeout for {check_in_date}: {exc}") from exc

        price = extract_lowest_price(text)
        if price is None:
            raise ValueError(f"Qunar price not found for {check_in_date}")
...
def _factory(context: ScraperContext) -> QunarScraper:
    return QunarScraper(default_room_name=context.default_room_name)


register_real_scraper("qunar", _factory)
